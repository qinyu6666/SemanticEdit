...
from models.utils import StyledConv, FixedStyledConv, ToRGB, PixelNorm, EqualLinear, ConvLayer, ResBlock, PositionEmbedding
...
class SemanticGenerator(nn.Module):
    def __init__(self, size=256, style_dim=512, n_mlp=8, channel_multiplier=2,
        ...
        layers = [PixelNorm()] #归一化
        for i in range(n_mlp):
            layers.append(
                EqualLinear(style_dim, style_dim, 
                    lr_mul=lr_mlp, activation='fused_lrelu')  #8个全连接层
            )
    ...
    def forward(self,
        ...
        seg_coarse = 2*seg_coarse-1 # normalize to [-1,1]        2,13,64,64
