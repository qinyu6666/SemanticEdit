...
def generate(model, styles, mean_latent=None, truncation=1.0, batch_size=1, *args, **kwargs):
    images, segs = [], []
    for head in range(0, styles.size(0), batch_size):
...
#component_dict_celeba = {1:"face", 2:"eye", 3:"eyebrow", 4:"mouth", 5:"nose",6:"ear", 7:"hair",8:"neck",}
# 找出眼睛
def tensor2segmaskeye(sample_seg):
    sample_seg = torch.argmax(sample_seg, dim=1)
    sample_mask = torch.ones(sample_seg.shape[0],sample_seg.shape[1], sample_seg.shape[2] )
    sample_mask[sample_seg==2] = 0
    return sample_mask
