...
from visualize.utils import generate, cubic_spline_interpolate,tensor2image
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        images, segs = generate(model, styles, randomize_noise=False)
    ...
    with torch.no_grad():
        ...
        styles_new = styles.repeat(args.sample, 1, 1) # 10,28,512
        mix_styles = model.style(torch.randn(args.sample, 512, device=args.device))  # 10,512
        mix_styles[-1] = mix_styles[0]
        # mix_styles = args.truncation * mix_styles + (1-args.truncation) * mean_latent.unsqueeze(0)
        mix_styles = mix_styles.unsqueeze(1).repeat(1,model.n_latent,1)  # 10,28,512
        # styles_new[:,latent_index] = mix_styles[:,latent_index]
        styles_new[:,6] = mix_styles[:,6]

        for i in range(10):
            # images, segs = generate(model, styles_new[[i]], mean_latent=mean_latent, randomize_noise=False)
            # images_, segs_ = model([styles_new[i:i+1]], input_is_latent=True,truncation=1., truncation_latent=mean_latent)
            images_, segs_ = model([styles_new[i:i+1]], input_is_latent=True,truncation=1.)

            images_ = tensor2image(images_)
            imageio.imwrite(f'{args.outdir}/image_{i}.jpeg', images_[0])



            # 以下为生成视频
            # styles_new = cubic_spline_interpolate(styles_new, step=args.steps) # 160,28,512
            ...
            # imageio.mimwrite(f'{args.outdir}/{6}.mp4', frames, fps=20)
            # # print(f"{args.outdir}/{latent_index:02d}_{latent_name}.mp4")
            # print(f"{args.outdir}/{6}.mp4")
        #
        # for i in range(1,10):
        #     styles_new1 = cubic_spline_interpolate(styles_new, step=args.steps)  # 160,28,512
        #     images_, segs_ = model([styles_new1[i*15]], input_is_latent=True, truncation=1., truncation_latent=mean_latent)
        #     imageio.imwrite(f'{args.outdir}/image_{i*15}.jpeg', images[0])





            # 在dim=1上拼接 [512,512,3] cat [512,512,3] -> [512,1024,3]